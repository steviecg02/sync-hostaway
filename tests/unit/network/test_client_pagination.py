...
def test_fetch_paginated_multiple_pages(mock_fetch_page: Mock) -> None:
    ...
    mock_fetch_page.side_effect = [
        {"result": [{"id": 1}], "count": 3, "limit": 1},
        {"result": [{"id": 2}], "count": 3, "limit": 1},
        {"result": [{"id": 3}], "count": 3, "limit": 1},
    ]

    result = fetch_paginated("listings", token="dummy-token", limit=1)
...
@patch("sync_hostaway.network.client.fetch_page")
def test_fetch_paginated_single_page(mock_fetch_page: Mock) -> None:
    """
    ...
    result = fetch_paginated("listings", token="dummy-token")
...
def test_fetch_page_handles_429_retry(mock_get: Mock) -> None:
    ...
    result = fetch_page("listings", token="dummy-token", page_number=0)
